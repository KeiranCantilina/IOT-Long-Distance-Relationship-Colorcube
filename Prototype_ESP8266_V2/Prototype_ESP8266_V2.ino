/*This is the second generation codebase for the LED Relationship Light Project.
...
/********************* RGB and Button Init ****************************/
  ...
  uint32_t red = RGBlight.gamma32(RGBlight.Color(255, 0, 0));
  RGBlight.setPixelColor(0,red);
  RGBlight.show();
...
void loop() {
  ...
  while (flag == 0 && (subscription = mqtt.readSubscription(1000))) {
    if (subscription == &subscribe_feed) {
      ...
      uint32_t color = RGBlight.gamma32(RGBlight.Color(string_to_red((char *)subscribe_feed.lastread)-111,string_to_green((char *)subscribe_feed.lastread)-111,string_to_blue((char *)subscribe_feed.lastread)-111));
      RGBlight.setPixelColor(0,color);
      RGBlight.show();
    }  
  }
  ...
  if (times_run == 0){
    uint32_t blue = RGBlight.gamma32(RGBlight.Color(0, 0, 255));
    RGBlight.setPixelColor(0,blue);
    RGBlight.show();
...
void MQTT_connect() {
  ...
  while ((ret = mqtt.connect()) != 0) { // connect will return 0 for connected
       ...
       retries--;
       if (retries == 0) {
         uint32_t red = RGBlight.gamma32(RGBlight.Color(255, 0, 0)); /// Red light for error
         RGBlight.setPixelColor(0,red);
         RGBlight.show();
         while (1);
       }
